...
void
epad_dpsta_link_monitor(uint8 *pData)
{
	...
	if ((event->bsscfgidx == 0) && ntohl(event->event_type) == WLC_E_LINK) {
		FILE *fp;
		int dpsta_enable = 0;

		fp = fopen("/proc/dpsta/enable", "r");
		if (fp != NULL) {
			dpsta_enable = fgetc(fp) - '0';
		}
		fclose(fp);
		...
		if (dpsta_enable) {
			if (ntohs(event->flags) & WLC_EVENT_MSG_LINK) {
				char *tok;
				char up_intfs[IFNAMSIZ * 2] = {0};

				fp = fopen("/proc/dpsta/up_intfs", "r");
				if (fp != NULL)
					fgets(up_intfs, sizeof(up_intfs), fp);
				fclose(fp);

				/* When upstream link up, flush fc entries that are related
				 * to dpsta up_intfs. This can prevent upstream kept at
				 * other link up radio.
				 */
				if (up_intfs[0] != '\0') {
					tok = strtok(up_intfs, " \n");
					while (tok != NULL) {
						eval("fc", "flush", "--if", tok);
						tok = strtok(NULL, " \n");
					}
				}
			} else {
				/* When upstream link down, flush fc entries that is related
				 * to cb->ifname. This can prevent upstream kept at current
				 * link down radio.
				 */
				eval("fc", "flush", "--if", event->ifname);

				/* Also flush cached stalist of dpsta */
