...
static CmsRet rclPeriodicstat_sendSampleSetMsg(CmsMsgType msgType,  const InstanceIdStack *iidStack, const _SampleSetObject *smpObj);

static CmsRet rclPeriodicstat_sendParameterMsg(CmsMsgType msgType,
      const InstanceIdStack *smpsetIidStack, const _SampleSetObject *smpObj,
      const InstanceIdStack *paramIidStack, const _SampleParameterObject *parameterObj);

static int rclPeriodicstat_checkRestartSampleSet(const _SampleSetObject *newObj, const _SampleSetObject *currObj);
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   PeriodicStatObject *periodicObj = NULL;

   if ( ADD_NEW(newObj, currObj) )
   {
      rut_modifyNumSampleSets(iidStack, 1);
   }
   ...
   ret = cmsObj_getAncestor(MDMOID_PERIODIC_STAT, MDMOID_SAMPLE_SET, &ancestorIidStack, (void **)&periodicObj);
   if ( ret != CMSRET_SUCCESS)
   {
      cmsLog_error("could not find ancestor obj %d (decendent oid %d) iidStack=%s",
                   MDMOID_SAMPLE_SET, MDMOID_SAMPLE_PARAMETER,
   ...
   needRestart = rclPeriodicstat_checkRestartSampleSet(newObj, currObj);
   ...
   if ( newObj != NULL )
   {
      ...
      {
         needRestart = FALSE;
         ret = rclPeriodicstat_sendSampleSetMsg(CMS_MSG_PERIODICSTAT_PAUSE_SAMPLESET, iidStack, currObj);
      }
      ...
      {
         needRestart = FALSE;
         ret = rclPeriodicstat_sendSampleSetMsg(CMS_MSG_PERIODICSTAT_FORCE_SAMPLE, iidStack, newObj);
      }
   ...
   {
      needRestart = FALSE;
      ret = rclPeriodicstat_sendSampleSetMsg(CMS_MSG_PERIODICSTAT_STOP_SAMPLESET, iidStack, currObj);
   }

   if ( needRestart )
   {
      ret = rclPeriodicstat_sendSampleSetMsg(CMS_MSG_PERIODICSTAT_START_SAMPLESET, iidStack, newObj);
   }
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   CmsRet ret = CMSRET_SUCCESS;
   InstanceIdStack ancestorIidStack = *iidStack;
   SampleSetObject *smpsetObj = NULL;

   if ( ADD_NEW(newObj, currObj) )
   {
      rut_modifyNumParameters(iidStack, 1);
   }
   ...
   ret = cmsObj_getAncestor(MDMOID_SAMPLE_SET, MDMOID_SAMPLE_PARAMETER, &ancestorIidStack, (void **)&smpsetObj);
   if ( ret != CMSRET_SUCCESS)
   {
      cmsLog_error("could not find ancestor obj %d (decendent oid %d) iidStack=%s",
                   MDMOID_SAMPLE_SET, MDMOID_SAMPLE_PARAMETER,
   ...
   if ( ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj) )
   {
      ret = rclPeriodicstat_sendParameterMsg(CMS_MSG_PERIODICSTAT_ADD_PARAMETER, &ancestorIidStack, smpsetObj, iidStack, newObj); 
   }

   /* Turn off a periodic statistics */
   if ( DELETE_OR_DISABLE_EXISTING(newObj, currObj) )
   {
      ret = rclPeriodicstat_sendParameterMsg(CMS_MSG_PERIODICSTAT_DELETE_PARAMETER, &ancestorIidStack, smpsetObj, iidStack, currObj);
   }
