...
/***************************************************************************
 * The BOLT implementation of ITC/RPC is unique. In order to provide continuity
...
int rpc_tunnel_init(enum rpc_tunnel_idx idx, bool handshake)
{
	...
	rpc_init();
	printf("ARMTF RPC: initializing %s tunnel... %d/%d\n", ft->name, ft->tx_fifo_idx, ft->rx_fifo_idx);
	...
	if (ft->tx_fifo_h < 0 || ft->rx_fifo_h < 0) {
		printf("ARMTF RPC: unable to open %s tunnel FIFO %d/%d\n", ft->name, ft->tx_fifo_h, ft->rx_fifo_h);
		return -1;
	}
