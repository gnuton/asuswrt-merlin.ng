...
export HND-96766_BASE := HND_ROUTER=y HND_ROUTER_BE_4916=y PROFILE="96766GW" SW_HW_AUTH=y AMAS=y UTF8_SSID=y ETHOBD=y DWB=y CONNDIAG=y SAMBA3="3.6.x" OOKLA=y WL_SCHED_V3=y UBI=y UBIFS=y PSISTLOG=y

export RT-BE82U := $(HND-96766_BASE)
export RT-BE82U += BUILD_NAME="RT-BE82U" NVSIZE="128" DHDAP=n HND_WL=y REPEATER=y IPV6SUPP=y HTTPS=y ARM=y AUTODICT=y \
		BBEXTRAS=y USBEXTRAS=y EBTABLES=y MEDIASRV=y MODEM=y USB_WAN_BACKUP=y PARENTAL2=y ACCEL_PPTPD=y PRINTER=y WEBDAV=y SMARTSYNCBASE=y \
		USB="USB" APP="network" PROXYSTA=y DNSMQ=y BCMWL6=y BCMWL6A=y DISK_MONITOR=y OPTIMIZE_XBOX=y ODMPID=y BCMSMP=y XHCI=y DUALWAN=y \
		NEW_USER_LOW_RSSI=y OPENVPN=y TIMEMACHINE=y MDNS=y VPNC=y JFFS2LOG=y BWDPI=y LINUX_MTD="64" DEBUGFS=y SSH=y EMAIL=y \
		FRS_FEEDBACK=n SYSSTATE=y STAINFO=y CLOUDCHECK=n NATNL_AICLOUD=y REBOOT_SCHEDULE=y MULTICASTIPTV=y QUAGGA=y BCM_MUMIMO=y \
		LAN50="all" ATCOVER=y GETREALIP=y CFEZ=y ETLAN_LED=y TFAT=y NTFS="tuxera" HFS="tuxera" NEWSSID_REV2=y NEWSSID_REV4=y \
		NEW_APP_ARM=y NETOOL=y TRACEROUTE=y FORCE_AUTO_UPGRADE=n ALEXA=y IFTTT=n HD_SPINDOWN=y BCMEVENTD=y LETSENCRYPT=y VPN_FUSION=y \
		JFFS_NVRAM=y NVRAM_ENCRYPT=y IPSEC=STRONGSWAN IPSEC_SRVCLI_ONLY=SRV NATNL_AIHOME=y BCM_CEVENTD=y DBLOG=y CFGSYNC_LOCSYNC=y \
		CRASHLOG=y WATCH_REINIT=n BW160M=y BRCM_HOSTAPD=y IPERF3=y FRS_LIVE_UPDATE=n ASUSCTRL=y BCN_RPT=y BTM_11V=y INFO_EXAP=y \
		AVBLCHAN=y SW_CTRL_ALLLED=y ASD=y AHS=n INTERNETCTRL=y INSTANT_GUARD=y ACL96=y GOOGLE_ASST=y COMFW=y WIREGUARD=y \
		AMAS_ETHDETECT=y NFCM=n DNSQUERY_INTERCEPT=y OPEN_NAT=y UUPLUGIN=n CAPTCHA=y GAME_MODE=y FRONTHAUL_DWB=y OWE_TRANS=y \
		EXTEND_LIMIT=y BCMBSD_V2=y WIFI7=y PC_SCHED_V3=y MULTISERVICE_WAN=y MTLANCFG=y WPA3_ENTERPRISE=y SMART_HOME_MASTER_UI=y \
		AMAS_OB_DISABLE_DSAP=y HND_GPIO=y AUTO_WANPORT=y \
		NFS=n SNMPD=y TOR=y UPNPIGD2=y NANO=y NTPD=y IPSET=y VPN_FUSION_MERLIN=y