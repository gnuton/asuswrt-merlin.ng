...
export XT8PRO += BUILD_NAME="XT8PRO" SWITCH2="BCM53134" NVSIZE="128" DHDAP=y HND_WL=y DPSTA=n LACP=y WTFAST=n REPEATER=y \
		DISABLE_REPEATER_UI=y DISABLE_PROXYSTA_UI=y IPV6SUPP=y HTTPS=y ARM=y AUTODICT=y BBEXTRAS=y USBEXTRAS=y EBTABLES=y \
		 MEDIASRV=y MODEM=y USB_WAN_BACKUP=y PARENTAL2=y ACCEL_PPTPD=y PRINTER=y WEBDAV=y SMARTSYNCBASE=y USB="USB" \
		APP="network" PROXYSTA=y DNSMQ=y SHP=n BCMWL6=y BCMWL6A=y DISK_MONITOR=y BTN_WIFITOG=n OPTIMIZE_XBOX=y ODMPID=y \
		LED_BTN=n BCMSMP=y XHCI=y DUALWAN=y  NEW_USER_LOW_RSSI=y OPENVPN=y TIMEMACHINE=y MDNS=y VPNC=y JFFS2LOG=y BWDPI=y \
		DUMP_OOPS_MSG=n LINUX_MTD="64" DEBUGFS=y TEMPROOTFS=n SSH=y EMAIL=y FRS_FEEDBACK=y SYSSTATE=y ROG=n STAINFO=y \
		CLOUDCHECK=y NATNL_AICLOUD=y REBOOT_SCHEDULE=y MULTICASTIPTV=y QUAGGA=y WLCLMLOAD=n BCM_MUMIMO=y LAN50="all" \
		ATCOVER=y GETREALIP=y CFEZ=y ETLAN_LED=y TFAT=y NTFS="tuxera" HFS="tuxera" NEWSSID_REV2=y NEWSSID_REV4=y \
		NEWSSID_REV5=y NEW_APP_ARM=y VISUALIZATION=n BONDING=y BONDING_WAN=n NETOOL=y TRACEROUTE=y FORCE_AUTO_UPGRADE=y \
		ALEXA=y IFTTT=n SW_HW_AUTH=y HD_SPINDOWN=y ASPMD=n BCM_MEVENT=n BCM_APPEVENTD=n LETSENCRYPT=y VPN_FUSION=y \
		JFFS_NVRAM=y NVRAM_ENCRYPT=y IPSEC=STRONGSWAN IPSEC_SRVCLI_ONLY=SRV NATNL_AIHOME=y BCM_CEVENTD=y UTF8_SSID=y \
		AMAS=y DWB=y FRONTHAUL_DWB=y DBLOG=y ETHOBD=y CFGSYNC_LOCSYNC=y CONNDIAG=y NFCM=n CRASHLOG=y WATCH_REINIT=y \
		BW160M=y BRCM_HOSTAPD=y UUPLUGIN=y IPERF3=y MSSID_PRELINK=y FRS_LIVE_UPDATE=y BCM_CLED=y SINGLE_LED=y BCN_RPT=y \
		BTM_11V=y INFO_EXAP=y BCMEVENTD=y AMAS_ADTBW=y SW_CTRL_ALLLED=y AMAS_SYNC_2G_BW=y AMAZON_WSS=n AHS=y ASD=y \
		INTERNETCTRL=y AVBLCHAN=y ADTBW_RADAR=y INSTANT_GUARD=y AMAS_ETHDETECT=y RTL8221VB=y ACL96=y GOOGLE_ASST=y \
		EXTEND_LIMIT=y ASUSCTRL=y DHD_6715B0=y COMFW=y FRONTHAUL_AP_AUTO_OPT=y WIREGUARD=y FSCRYPTCTL=n AMAS_SS2=n \
		MTLANCFG=y CAPTIVE_PORTAL=y CP_FREEWIFI=y AMAS_SS2=y AMAS_CENTRAL_OPTMZ=y
export XT8PRO += TOR=y NTPD=y DNSPRIVACY=y DNSFILTER=y NTPD=y OPENSSL11=y NANO=y IFTTT=y FORCE_AUTO_UPGRADE=n FRS_FEEDBACK=n
export XT8PRO += UPNPIGD2=n VISUALIZATION=n WIREGUARD=y SNMPD=y